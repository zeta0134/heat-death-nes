...
def read_level(map_filename):
    ...
    # hack: only one of our layers contains valid tile data; the others are visual guides,
    # so ignore them
    only_layer = None
    layer_elements = map_element.findall("layer")
    for layer_element in layer_elements:
        if layer_element.get("name") == "Level Tiles":
    ...
    return Level(name=safe_label, width=map_width, height=map_height, tiles=only_layer)

def raw_level_bytes(tilemap):
  raw_bytes = []  
  for column in range(0, tilemap.width):
    for row in range(0, 16):
      tile = tilemap.tiles[row * tilemap.width + column]
      raw_bytes.append(tile.integer_properties["tile_index"])
  return raw_bytes

def write_level(tilemap, output_file):
    output_file.write(ca65_label("level_"+tilemap.name) + "\n")
    output_file.write("  .byte %s ; width\n" % ca65_byte_literal(tilemap.width))
    pretty_print_table(raw_level_bytes(tilemap), output_file, tilemap.height)
    
if __name__ == '__main__':
    # DEBUG TEST THINGS
    if len(sys.argv) != 3:
      print("Usage: convert_level.py input.tmx output.txt")
      sys.exit(-1)
    input_filename = sys.argv[1]
    output_filename = sys.argv[2]

    tilemap = read_level(input_filename)
